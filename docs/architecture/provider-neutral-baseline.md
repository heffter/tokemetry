...
| 5. Migration constraints and test-to-epic mapping | 60.5 | Complete |
...
### 5.1 Migration mechanics (`db/migrate.py`, `app.py`, `config.py`)

- **Alembic chain**: `0001_initial_schema` -> `0002_alert_dual_thresholds_state`
  -> `0003_app_settings` (current head `0003`). Naming is deterministic across
  dialects via the `base.py` convention (Section 1.1), so autogenerated
  migrations produce stable names on SQLite and Postgres.
- **Startup upgrade**: when `auto_migrate` is true (default, `config.py:49`) the
  app runs `upgrade_to_head(sync_database_url)` in its lifespan
  (`app.py:85-86`). Production may instead run migrations out-of-band and set
  `auto_migrate=false`.
- **Sync-URL derivation** (`config.py:101` `sync_database_url`): the async
  `database_url` is mapped to a sync driver for Alembic -
  `postgresql+asyncpg -> postgresql+psycopg`, `sqlite+aiosqlite -> sqlite`
  (`config.py:18-19`). Alembic always runs on the sync URL.
- **Two-engine consistency (FR-RET-007)**: SQLite (development/tests) and
  Postgres (production) MUST behave consistently. The `''`-sentinel grain,
  `JSON`/`JSONB` variant, and `ON CONFLICT` upserts (Sections 1.1-1.3) exist
  specifically to keep behavior identical across the two engines. Every
  migration must therefore be valid on both.

### 5.2 Reversibility and the both-engines test rule

- **Downgrade policy (NFR-REL-003)**: migrations MUST be reversible where
  practical. Migration 0001 has a full `downgrade()` (drops all tables in
  dependency order); 0002/0003 reverse their column/table additions.
- **Existing coverage** (`apps/server/tests/integration/test_migrations.py`):
  upgrade-creates-all-tables, downgrade-removes-domain-tables, and
  ORM/metadata-drift - but currently **SQLite only** (`_sqlite_url`).
- **Rule for Tasks 61-71**: every database-changing subtask MUST ship **upgrade
  and downgrade tests for both engines** (SQLite and Postgres). The current
  SQLite-only migration test must be extended to run against a Postgres engine
  (e.g. a CI service container or testcontainers) as part of the first
  schema-changing epic (TOK-3).

### 5.3 Backup and upgrade posture

- **Backup before every migration**: `deploy/backup.sh` performs a timestamped,
  compressed `pg_dump` with retention pruning (nightly in production). The
  operational rule for this program: **run a backup (this script or an
  equivalent `pg_dump`) immediately before applying any migration**, so a failed
  or unsatisfactory migration can be restored.
- **Maintenance-window upgrades are acceptable**: this is a single-instance
  deployment (one server, one Postgres). Brief downtime for migrate + restart is
  acceptable. **Rolling / zero-downtime upgrades are out of scope**; migrations
  need not be simultaneously compatible with old and new code running
  concurrently, though the v1 wire contract (Section 4) still holds across the
  upgrade.

### 5.4 Test-to-epic map

Disposition legend: **Green** = must stay green unchanged (v1 compatibility);
**Extend** = kept and extended for v2 features; **Supersede** = largely replaced
by the named epic. Epic numbers: TOK-2 (61) provider/mode, TOK-3 (62) usage
event v2 ledger, TOK-4 (63) source registry, TOK-5 (64) pricing rate card, TOK-6
(65) aiProviderProxy, TOK-7 (66) query API v2, TOK-8 (67) dashboard, TOK-9 (68)
alerts/data-quality, TOK-10 (70) retention/security, TOK-11 (71) OpenTelemetry,
TOK-12 (69) multi-provider limits.

**Anchor:** `apps/server/tests/integration/test_v1_golden.py` (added in 60.4) is
**Green - the compatibility anchor**: it must stay green, unedited, through every
epic until a formal v1 deprecation at Phase 7. A change to it is a deliberate
contract change, never incidental.

`packages/core/tests/unit/`:

| File | Disposition | Epic(s) |
| --- | --- | --- |
| `test_models.py` | Extend | TOK-3 (v2 event fields) |
| `test_claude_code_source.py` | Green, Extend | TOK-4 (source registry) |
| `test_registry.py` | Extend | TOK-2, TOK-4 |
| `test_fake_provider.py` | Extend | TOK-2 |
| `test_pricing.py` | Green, Extend | TOK-5 (multi-provider pricing) |
| `test_projects.py` | Green | - (grouping unchanged) |
| `test_core_package.py` | Green | - (package smoke) |

`apps/server/tests/unit/`:

| File | Disposition | Epic(s) |
| --- | --- | --- |
| `test_config.py` | Extend | TOK-2, TOK-10 (new settings) |
| `test_cost_engine.py` | Green, Extend | TOK-5 |
| `test_insights.py` | Green, Extend | TOK-8 |
| `test_report_rules.py` | Green, Extend | TOK-8 |
| `test_report_export.py` | Green, Extend | TOK-8 |
| `test_reset_rollover.py` | Green, Extend | TOK-12 |
| `test_session_stats.py` | Green | - |
| `test_quiet_hours.py` | Green, Extend | TOK-9 |
| `test_notifiers.py` | Green, Extend | TOK-9 |
| `test_security.py` | Green, Extend | TOK-10 |
| `test_server_package.py` | Green | - |

`apps/server/tests/integration/`:

| File | Disposition | Epic(s) |
| --- | --- | --- |
| `test_v1_golden.py` | **Green (anchor)** | none until Phase 7 |
| `test_ingest.py` | Green, Extend | TOK-3 (v2 ingest) |
| `test_query_api.py` | Green, Extend | TOK-7 |
| `test_queries.py` | Green, Extend | TOK-7 |
| `test_rollups.py` | Green, Extend | TOK-3 |
| `test_pricing_api.py` | Green, Extend | TOK-5 |
| `test_pricing_repo.py` | Green, Extend | TOK-5 |
| `test_litellm_sync.py` | Green, Extend | TOK-5 |
| `test_alerts_api.py` | Green, Extend | TOK-9 |
| `test_alert_engine.py` | Green, Extend | TOK-9 |
| `test_alert_rules.py` | Green, Extend | TOK-9 |
| `test_alert_seed.py` | Green, Extend | TOK-9 |
| `test_analytics.py` | Green, Extend | TOK-7, TOK-12 |
| `test_channels.py` | Green, Extend | TOK-9 |
| `test_migrations.py` | Extend | TOK-3 (+ Postgres, both-engines rule 5.2) |
| `test_spa_serving.py` | Green | - |
| `test_tokens_and_stream.py` | Green, Extend | TOK-7, TOK-10 |

`apps/collector/tests/`:

| File | Disposition | Epic(s) |
| --- | --- | --- |
| `unit/test_state.py` | Green, Extend | TOK-4 |
| `unit/test_uploader.py` | Green, Extend | TOK-6 |
| `unit/test_runner.py` | Green, Extend | TOK-4, TOK-6 |
| `unit/test_cli.py` | Green, Extend | TOK-4 |
| `unit/test_collector_config.py` | Green, Extend | TOK-4 |
| `unit/test_limits_anthropic.py` | Green, Extend | TOK-12 |
| `integration/test_claude_code_collection.py` | Green, Extend | TOK-4 |

`apps/dashboard/src/**/*.test.ts` (vitest):

| File | Disposition | Epic(s) |
| --- | --- | --- |
| `lib/format.test.ts` | Supersede | TOK-8 (generalized labels/windows) |
| `lib/filters.test.ts` | Extend | TOK-7, TOK-8 (provider dimension) |
| `api/client.test.ts` | Extend | TOK-7 |
| `lib/coverage.test.ts` | Green, Extend | TOK-8 |
| `lib/machines.test.ts` | Green | - |
| `lib/charts.test.ts` | Green | - |
| `lib/sparkline.test.ts` | Green | - |
| `lib/errors.test.ts` | Green | - |
| `lib/throttle.test.ts` | Green | - |
| `composables/useAsync.test.ts` | Green | - |

**Inventory totals** (for the PR checklist): core 7, server unit 11, server
integration 17 (incl. the new anchor), collector 7, dashboard vitest 10.
